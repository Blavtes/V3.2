...
public class SimpleCache<K, V> implements Cache<K, V>, Serializable {
    ...
    public static final int          DEFAULT_MAX_BYTE_SIZE = 1024 * 1024 * 5; // 5MB
     ...
     */
    public SimpleCache(){
        this(DEFAULT_MAX_ITEM_SIZE, DEFAULT_MAX_BYTE_SIZE);
    }
     ...
     */
    public SimpleCache(int maxItemSize, long maxByteSize){
        if (maxItemSize <= 0) {
        ...
        this.validTime = -1;
        this.cache = new ConcurrentHashMap<K, CacheObject<V>>(maxItemSize);
    }
    ...
    public CacheObject<V> put(K key, V value) {
        ...
        if (value instanceof String) {
            ...
            if (f != null && f.exists()) {
                obj.setLastUsedTime(f.lastModified());
            }
        }
        obj.setData(value);
    ...
    public CacheObject<V> removeByValue(V value) {
        CacheObject<V> obj = null;
        Set<K> keySet = cache.keySet();
        for(K k : keySet) {
            obj = cache.get(k);
            if (obj != null && obj.getData() == value) {
