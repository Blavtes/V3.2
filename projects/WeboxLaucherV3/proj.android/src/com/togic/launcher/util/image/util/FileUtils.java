...
public class FileUtils {
    ...
    public static boolean writeFile(String filePath, InputStreamAndLength obj) {
        if (obj == null || obj.stream == null || obj.length <= 0) {
            return false;
        }
        ...
        try {
            ...
            byte data[] = new byte[1024];
            int length = -1;
            while ((length = obj.stream.read(data)) != -1) {
                o.write(data, 0, length);
            }
            o.flush();
            ...
            if (obj.length == f.length()) {
                ...
                Log.w("FileUtils", "&&&&&&&&& need write " + obj.length
                        + " bytes, but only write " + f.length()
                        + " bytes to file " + filePath);
                return false;
            }
        } catch (FileNotFoundException e) {
            throw new RuntimeException("FileNotFoundException occurred. ", e);
