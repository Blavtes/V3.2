...
//    this->addChild(m_bottomBar,1);
	...
	listener->onKeyPressed = CC_CALLBACK_3(MainLayer::onKeyPressed,this);
	listener->onKeyReleased= CC_CALLBACK_3(MainLayer::onKeyReleased,this);
	CCDirector::getInstance()->getEventDispatcher()->addEventListenerWithSceneGraphPriority(listener,this);

	return true;
}

void MainLayer::onKeyPressed(EventKeyboard::KeypadEventCode keyCode, EventKeyboard::KeypadEventType eventType, Event* event)
{
	if(m_focusHelper != NULL)
	{
		m_focusHelper->onKeyPressed(keyCode,eventType,event);
	}
}

void MainLayer::onKeyReleased(EventKeyboard::KeypadEventCode keyCode, EventKeyboard::KeypadEventType eventType, Event* event)
{
	if(m_focusHelper != NULL)
	{
		m_focusHelper->onKeyReleased(keyCode,eventType,event);
	}
	if(keyCode == EventKeyboard::KeypadEventCode::kCodeMenu )
	{
